    ...
    public:
        ...
        void Task(long length) {
            ...
            while (length > 0) {
                ...
                if (len < length) {
                    res += str.substr(vertexes[i].l, len);
                    length -= len;
                } else {
                    res += str.substr(vertexes[i].l, length);
                    return;
                }
            }
        }
...
int main() {
    ...
    std::string s2 = min_cyclic_shift(str);
    end2 = std::chrono::system_clock::now();

    std::cout << "lab5: " << std::chrono::duration<double>(end - start).count() << " s" << std::endl;
    std::cout << "naive: " << std::chrono::duration<double>(end2 - start2).count() << " s" << std::endl;
    std::cout << std::endl;
