...
class TLongArithmetic {
    ...
    public:
        ...
        TLongArithmetic(const std::string &str);
        void Clear();
        bool IsOk() const;
        void DeleteZeros();
...
namespace NLongArithmetic {
    ...
    TLongArithmetic::TLongArithmetic(const std::string &str) {
        data.clear();
        this->Init(str);
    }
...
int main() {
    ...
    mpz_t a1, b1, c1, z;
    char action;
    std::chrono::system_clock::time_point start, start1;
    std::chrono::system_clock::time_point end, end1;
    double time = 0;
    double time1 = 0;
    while (std::cin >> a0 >> b0 >> action){
        long_type a(a0), b(b0), c;
        mpz_init_set_str(a1, a0.c_str(), 10000);
        mpz_init_set_str(b1, b0.c_str(), 10000);
        mpz_init_set_str(z, 0, 10000);
        c.Clear();
        switch (action) {
        case '+':
        ...
        case '/':
            ...
            start1 = std::chrono::system_clock::now();
            if (mpz_cmp(a1, b1) >= 0 && mpz_cmp(b1, z) != 0) mpz_tdiv_q(c1, a1, b1);
            end1 = std::chrono::system_clock::now();
            break;
    ...
    std::cout << "time_my_lab: " << time << " s " << " time_gmp: " << time1 << " s"<< std::endl;
}